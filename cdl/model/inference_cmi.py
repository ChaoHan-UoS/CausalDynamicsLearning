...
class InferenceCMI(Inference):
    ...
    def forward_with_feature(self, feature, actions, mask=None,
        ...
        """
        :param feature: (bs, feature_dim) if state space is continuous else [[(bs, feature_i_dim)] * feature_dim] * 3
            notice that bs can be a multi-dimensional batch size
        ...
        full_feature = feature[0] if "full" in forward_mode else None
        masked_feature = feature[1] if "masked" in forward_mode else None
        causal_feature = feature[2] if "causal" in forward_mode else None
        ...
        if not self.continuous_action:
            actions = F.one_hot(actions.squeeze(dim=-1).long(),
                                self.action_dim).float()  # (bs, n_pred_step, action_dim)
    ...
    def rec_loss_from_feature(self, feature, rew):
        ...
        # rew_one_hot = F.one_hot(rew_unnorm.long(), self.feature_inner_dim[0] + 1).float()
        # rew_one_hot = rew_one_hot.permute(0, 2, 1) if len(rew_one_hot.shape) == 3 else rew_one_hot
        ...
        if not self.training:
            ...
            rec_loss = (rew_feature_ids == rew_unnorm).sum() / rew_feature_ids.shape[0]
        else:
    ...
    def prediction_loss_from_multi_feature(self, pred_next_feature, next_rew):
        ...
        pred_loss = full_pred_loss.clone()
    ...
    def update(self, obs, actions, next_obses, rew, next_rews, hidden_label, eval=False):
        ...
        :param rew: (bs, stack_num, reward_dim)
        :param next_rews: (bs, n_pred_step, reward_dim)
        :param hidden_label: (bs, hidden_dim)
        ...
        seq_len = obs.shape[1]
        s_1 = None
        rec_pred_loss_seq, full_pred_loss_rew_seq, causal_pred_loss_rew_seq = (
        ...
        pred_next_feature = [[torch.zeros(bs, 1, self.feature_inner_dim[0])] * self.feature_dim] * 3
        ...
        for t in range(seq_len):
            ...
            feature, target, s_1 = self.encoder(None, obs[:, t: t + 1], t, s_1)

            # Negative features
            # feature_neg, target_neg, s_1 = self.encoder(pred_next_feature[0], obs_neg[:, t: t+1], t, s_1)
            feature_neg, target_neg, s_1 = self.encoder(None, obs_neg[:, t: t + 1], t, s_1)

            # [[(bs, 1, num_colors)] * num_objects] * 3
            feature_unsqueeze_multi = [[feature_i.unsqueeze(1) for feature_i in feature]] * 3

            if len(self.hidden_objects_ind) == 0:
                # for fully observable env
                for forward_mode1, forward_mode2 in zip(pred_next_feature, feature_unsqueeze_multi):
                    forward_mode2[1] = forward_mode1[1].clone().detach()
                # full observations as samples in NLL of transition
                feature_unsqueeze = [feature_i.unsqueeze(1) for feature_i in feature]
            else:
                for forward_mode1, forward_mode2 in zip(pred_next_feature, feature_unsqueeze_multi):
                    forward_mode2[self.hidden_objects_ind[0]] = forward_mode1[
                        self.hidden_objects_ind[0]].clone().detach()
                feature_unsqueeze = feature_unsqueeze_multi[0]
            ...
            if 0 < t <= seq_len - 1:
                ...
                pred_next_feature_target = [forward_mode1 + forward_mode2
                                            ...
                                            zip(feature_unsqueeze_multi, target_unsequeeze_multi)]
                ...
                # NLL transition loss for positive features
                pred_next_dist_sliced = [
                    forward_mode[:self.hidden_objects_ind[0]] + forward_mode[self.hidden_objects_ind[0] + 1:]
                    for forward_mode in pred_next_dist
                ]
                feature_unsqueeze_sliced = (feature_unsqueeze[:self.hidden_objects_ind[0]]
                                            + feature_unsqueeze[self.hidden_objects_ind[0] + 1:])
                pred_loss, loss_detail = self.prediction_loss_from_multi_dist(
                    pred_next_dist_sliced, feature_unsqueeze_sliced)
                ...
                # # MSE loss for negative features
                # feature_one_hot_neg = self.sample_from_distribution(feature_neg)
                # # (num_objects, bs, num_colors)
                # pred_loss_full_neg = self.loss_mse(torch.stack(feature_one_hot), torch.stack(feature_one_hot_neg))
                # pred_loss_full_neg = pred_loss_full_neg.sum(dim=(0, 2)).mean()
                # pred_loss_full_neg = torch.max(torch.zeros_like(pred_loss_full_neg),
                #                                self.cmi_params.hinge - pred_loss_full_neg)
                # pred_loss_seq_neg += pred_loss_full_neg

                # CE loss for negative features
                feature_one_hot_neg = self.sample_from_distribution(feature_neg)
                # (bs, num_colors, num_objects)
                feature_one_hot_neg = torch.stack(feature_one_hot_neg, dim=-1)
                feature_logits = torch.stack(feature, dim=-1)
                # (bs, num_objects)
                pred_loss_full_neg = self.loss_ce(feature_logits, feature_one_hot_neg)
                pred_loss_full_neg = pred_loss_full_neg.sum(dim=-1).mean()
                pred_loss_full_neg = torch.max(torch.zeros_like(pred_loss_full_neg),
                                               self.cmi_params.hinge - pred_loss_full_neg)
                pred_loss_seq_neg += pred_loss_full_neg
                """
            ...
            feature_multi = [
                [feature_i.squeeze(dim=1) for feature_i in forward_mode]
                for forward_mode in feature_unsqueeze_multi
            ]
            ...
            """ transition the logits instead of one-hot samples of the hidden
            pred_next_feature = [
                [dist_i.logits for dist_i in forward_mode]
                for forward_mode in pred_next_dist
            ]
            """
        ...
        # pred_loss_detail["pred_loss_neg"] = pred_loss_seq_neg
        ...
        # loss = 20 * rec_loss_seq + pred_loss_seq_neg
        # loss_detail = {**pred_loss_detail, **rec_loss_detail}
        # loss = 20 * rec_loss_seq if self.training else rec_loss_seq / seq_len
        # loss_detail = {**rec_loss_detail}
        loss = pred_loss_seq + 20 * rec_pred_loss_seq
        loss_detail = {**loss_detail, **rec_pred_loss_detail}
        ...
        with torch.no_grad():
            for i in range(feature_dim):
                ...
                for t in range(seq_len):
                    ...
                    # Loss evaluated at last step
                    if t == seq_len - 1:
                        target_unsqueeze_multi = [[target_i.unsqueeze(1) for target_i in target]] * 3
                        """ transition the logits instead of one-hot samples of the hidden
                        feature_unsqueeze[self.hidden_objects_ind[0]] = self.sample_from_distribution(
                            [feature_unsqueeze[self.hidden_objects_ind[0]]])[0]
                        """
                        if i == 0:
                            # Predicted reward loss
                            # [(bs, 1, num_colors)] * 2 * num_objects] * 3
                            pred_next_feature_target = [forward_mode1 + forward_mode2
                                                        for forward_mode1, forward_mode2 in
                                                        zip(feature_unsqueeze_multi, target_unsqueeze_multi)]
                            rec_pred_loss, rec_pred_loss_detail = \
                                self.prediction_loss_from_multi_feature(pred_next_feature_target, rew[:, t: t + 1])
                            ...
                            full_pred_loss, masked_pred_loss, eval_pred_loss = [
                                self.prediction_loss_from_dist(
                                    pred_next_dist_i, feature_unsqueeze, keep_variable_dim=True)
                                for pred_next_dist_i in pred_next_dist
                            ]
                        else:
                            # NLL transition loss
                            masked_pred_loss = self.prediction_loss_from_dist(
                                pred_next_dist[1], feature_unsqueeze, keep_variable_dim=True)
                        break
