...
class RecurrentEncoder(RNN):
    ...
    def forward(self, obs, s_0=None):
        ...
        if self.continuous_state:
            ...
            obs_obs = torch.stack(obs[:-1], dim=0)  # (num_dset_observables, bs, stack_num, (n_pred_step), num_colors)

            # slice in action and reward from observation
            obs_act = obs[-1]  # (bs, stack_num, (n_pred_step), action_dim / reward_dim)
            # mask out the last action / reward in the stacked obs
            obs_act[:, -1] = obs_act[:, -1] * 0
            ...
            if obs_obs_dims == 5:
                ...
                # [(bs, stack_num, num_dset_observables * num_colors + action_dim)] * n_pred_step
