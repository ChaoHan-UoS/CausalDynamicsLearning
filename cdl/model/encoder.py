...
class RecurrentEncoder(Recurrent):
    ...
    def __init__(self, params, *args, **kwargs):
        ...
        if not self.continuous_state:
            self.feature_inner_dim = np.concatenate([params.obs_dims[key] for key in self.keys])
        self.feature_inner_dim_remapped = np.append(self.feature_inner_dim, [params.action_dim, params.env_params.chemical_env_params.max_steps])
    ...
    def forward(self, obs, s_0=None, info={}, detach=False):
        ...
        if self.continuous_state:
            ...
            obs = [obs_k_i
                   ...
                   for obs_k_i in torch.unbind(obs[k], dim=-1)]
            obs = [F.one_hot(obs_i.long(), obs_i_dim).float() if obs_i_dim > 1 else obs_i.unsqueeze(dim=-1)
                   for obs_i, obs_i_dim in zip(obs, self.feature_inner_dim_remapped)]
            obs_obs = torch.stack(obs[:-2], dim=0)  # shape (num_observed_objects, bs, stack_num, num_colors) or (num_observed_objects, bs, stack_num, 1, num_colors)
            obs_obs_forward = torch.unbind(obs_obs[:, :, -1])
            ...
            obs_act, obs_step = obs[-2], obs[-1]
            obs_act = obs_act.reshape(obs_act.shape[:2] + (-1,))
            obs_step = obs_step.reshape(obs_step.shape[:2] + (-1,))
            # concatenate one-hot observation, action and episode step along the last dim
            obs = torch.cat([obs_obs, obs_act, obs_step], dim=-1) # shape (bs, stack_num, num_observed_objects * num_colors + action_dim + max_steps)

            obs, s_n = super().forward(obs, s_0, info)  # obs with shape (bs, logit_shape)
            # max_ind = torch.argmax(obs.reshape(-1, self.num_objects, self.num_colors), dim=-1)
            # obs = F.one_hot(max_ind, num_classes=self.num_colors)  # shape (bs, num_objects, num_colors)
            # obs = obs.reshape(-1, self.num_hidden_objects, self.num_colors)  # shape (bs, num_hidden_objects, num_colors)
            # obs = F.softmax(obs.reshape(-1, self.num_hidden_objects, self.num_colors), dim=-1)  # shape (bs, num_hidden_objects, num_colors)
            # max_ind = torch.argmax(obs.reshape(-1, self.num_hidden_objects, self.num_colors), dim=-1)
            # obs = F.one_hot(max_ind, num_classes=self.num_colors).float()   # shape (bs, num_hidden_objects, num_colors)
            # overwrite some observations for out-of-distribution evaluation
            if not getattr(self, "chemical_train", True):
                assert self.params.env_params.env_name == "Chemical"
                assert self.params.env_params.chemical_env_params.continuous_pos
            ...
            obs = obs_obs_forward + torch.unbind(obs)  # concatenate RNN's output and FNN's
            # obs = torch.cat((obs, obs_obs_forward), dim=0)
            # obs = F.softmax(obs, dim=-1)  # shape (num_objects, bs, num_colors)
...
def make_encoder(params):
    ...
    if encoder_type == "recurrent":
        ...
        obs_shape = len(params.obs_keys) * chemical_env_params.num_colors + params.action_dim + chemical_env_params.max_steps
